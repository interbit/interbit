...
assigned only to the `root` role.
...
A read join means that one blockchain, the _sender_, has access to a
copy of the state -- the collection of data stored on the blockchain --
of another blockchain, the _recipient_, at a specific point in time. Any
updates to the recipient's state is not reflected in the sender's
blockchain.

A write join means that, in addition to the capabilities of a read join,
that the sender receives updates whenever the recipient's state
changes.

After a join has been established, the join can be revoked and the
sender no longer has access to the recipient's state.
