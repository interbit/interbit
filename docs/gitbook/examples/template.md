...
This walk-through starts where the [Getting
Started](../getting-started/README.md) guide left off.
...
1. Run a specific covenant on it. The covenant hash and therefore the
   code must be known to the host.

1. Allow the hosting chain to form all of its blocks by including it's
   public key as root and as the block master.

1. Know the chain ID of its host and send the host its sponsorship
   request.
...
1. We dispatch an action into the redux store to tell it which chain we
   want to see first.
