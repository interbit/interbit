...
const { defaultManifest } = require('../../../../interbit/src/tests/testData')
...
describe('rootCovenant', () => {
  ...
  it.only('SET_MANIFEST updates manifest', () => {
  ...
  it('SET_MANIFEST throws with incorrect manifest', () => {
    ...
    const manifest = {
      public: defaultManifest.manifest.interbitRoot.chains.public
    }
    const action = rootCovenant.actionCreators.setManifest(manifest)

    should.throws(() => {
      rootCovenant.reducer(state, action)
    }, /^This chain does not match the given manifest/)
  })
